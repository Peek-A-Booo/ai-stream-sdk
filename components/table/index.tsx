...
export function OptionTable({
  ...
  const th1 = language === 'zh-CN' ? '选项' : 'Option'
  const th2 = language === 'zh-CN' ? '类型' : 'Type'
  const th3 = language === 'zh-CN' ? '描述' : 'Description'
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b py-4 text-left dark:border-neutral-700">
            <th className="py-2 font-semibold">{th1}</th>
            <th className="py-2 pl-6 font-semibold">{th2}</th>
            <th className="px-6 py-2 font-semibold">{th3}</th>
          </tr>
        </thead>
